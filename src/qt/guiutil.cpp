...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("futurecash"))
        return false;
...
{
    // Convert futurecash:// to futurecash:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("futurecash://"))
    {
        uri.replace(0, 12, "futurecash:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FutureCash.lnk";
}
...
{
    return GetAutostartDir() / "futurecash.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FutureCash-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  futurecash-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("FutureCash-Qt"));
